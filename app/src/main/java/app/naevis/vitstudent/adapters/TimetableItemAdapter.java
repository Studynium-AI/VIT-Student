...
/**
 * ┬─── Timetable Hierarchy
 * ├─ {@link app.naevis.vitstudent.fragments.HomeFragment}
...
public class TimetableItemAdapter extends RecyclerView.Adapter<TimetableItemAdapter.ViewHolder> {
    ...
    public TimetableItemAdapter(List<Timetable.AllData> timetable, int status) {
        this.timetable = Timetable.combineLabs(timetable);
        this.status = status;
    }
            ...
            coursesDao
                    ...
                    .subscribe(new SingleObserver<Course.AllData>() {
                        ...
                        public void onSuccess(@io.reactivex.rxjava3.annotations.NonNull Course.AllData course) {
                            ...
                            app.naevis.vitstudent.helpers.TaskDialogHelper.loadTasksForBottomSheet(bottomSheetLayout, course.courseCode, appDatabase);

                            View btnAddTask = bottomSheetLayout.findViewById(R.id.btn_add_task);
                            btnAddTask.setOnClickListener(v -> {
                                app.naevis.vitstudent.helpers.TaskDialogHelper.showAddTaskDialog(context, course.courseCode, course.courseTitle, () -> {
                                    app.naevis.vitstudent.helpers.TaskDialogHelper.loadTasksForBottomSheet(bottomSheetLayout, course.courseCode, appDatabase);
                                });
                            });
